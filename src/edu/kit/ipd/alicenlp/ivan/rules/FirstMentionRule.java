...
public class FirstMentionRule implements IDocumentRule {
	...
	public boolean apply(Annotation doc, boolean canWrite) throws IvanException {
		...
		for (Pair<String, String> thing : state.getEntityNames()) {
			...
			if (infos == null)
				throw new IvanException("State is inconsistent. Name '" + handle + "' was given but has no attached entity information.");
			...
			for (int i = 0; i < infos.size(); i++) {
				...
				// find first location
				if (ei.hasLocation()) {
					if (earliestLocation == null || ei.getEntitySpan().isBefore(earliestLocation.getEntitySpan())) {
						earliestLocation = ei;
					}
				}
				// find first direction
				if (ei.hasDirection()) {
					if (earliestDirection == null || ei.getEntitySpan().isBefore(earliestDirection.getEntitySpan())) {
			...
			if (earliestDirection != earliestLocation && earliestLocation != null)
				resultSpans.add(earliestMention.getEntitySpan().start());
		}
		...
		for (int i = 0; i < sentences.size(); i++) {
			...
			Integer sentenceEnd = s.get(CoreAnnotations.CharacterOffsetEndAnnotation.class);
			boolean addme = false;
