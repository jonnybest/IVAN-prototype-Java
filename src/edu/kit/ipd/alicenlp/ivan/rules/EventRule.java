...
public class EventRule implements ISentenceRule {
	...
	/** This method constructs a list of keywords which signal an event.
	 *  In particular, this means "appear", "disappear" and all their synonyms.
	...
	private static void setupSenses() throws JWNLException {
		...
		// synonyms of appear
		for (PointerTarget synonym : appear.getSynset().getTargets(PointerType.HYPONYM)) {
		...
		for (PointerTarget synonym : disappear.getSynset().getTargets(PointerType.HYPONYM)) {
			allowedSenses.add(synonym.getSynset());
		}
	}
	...
	public boolean apply(CoreMap Sentence) {
		List<CoreLabel> verbs = new ArrayList<CoreLabel>(); // get verbs for this sentence
		...
		for (CoreLabel v : verbs) {
			try {
				...
				/* This loop is where the real magic happens. If the word <code>v</code>
				 * can be interpreted as meaning anything on <code>allowedSenses</code>,
				 * however unlikely, that means this one qualifies as event.
				 */
				for (Synset sense : wnw.getSenses()) {
					if(allowedSenses.contains(sense))
					{
						double threshold = 0.35;
						// likelihood-based check. everything less likely than .8 is skipped.
						...
						if(new SentenceFrameRule(sense.getVerbFrames()).apply(Sentence))
							return true;
						else 
							return false;
					}
